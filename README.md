...
> ℹ️ 当前版本 **v0.2.1**:网页接口 + 三种识别 + 结果缓存 + 多方式合并 + 调试标注图 + 监控指标,默认 163 个测试全绿，另有 8 个真实模型冒烟测试。模板匹配开箱即用(自带示例),YOLO 自带通用模型 `yolov8n`;要用自己的模型/模板时从 `configs/*.yaml.example` 复制一份再改。详见 [开发文档](#开发文档)。
...
AI 助手规则以 `CLAUDE.md` 为唯一权威源；`AGENTS.md` 只作为 Codex 入口，要求 Codex 读取并遵守 `CLAUDE.md`，不复制规则正文。
`.claude/`、`.agent/`、`.agents/`、`.codex/`、`.Codex/` 是本机工具私有配置目录（权限、缓存、机器路径等），保持 gitignore，不跨工具同步。
要把这套规范复用到其他仓库，可参考仓库内的 skill：[agent-rules-single-source](skills/agent-rules-single-source/SKILL.md)。
...
- [x] 真实模型端到端冒烟测试与标准样例回归（默认 163 个测试全绿，另有 8 个真实冒烟测试，含真实游戏截图回归）
