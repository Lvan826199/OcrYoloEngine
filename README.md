...
> ℹ️ 当前版本 **v0.2.1**:网页接口 + 三种识别 + 结果缓存 + 多方式合并 + 调试标注图 + 监控指标,158 个测试全部通过。模板匹配开箱即用(自带示例),YOLO 自带通用模型 `yolov8n`;要用自己的模型/模板时从 `configs/*.yaml.example` 复制一份再改。详见 [开发文档](#开发文档)。
...
AI 助手规则以 `CLAUDE.md` 为源文件；改完后运行 `uv run python scripts/sync_agent_docs.py` 生成 `AGENTS.md`，Codex/Claude 两边就不会漂移。
...
- [x] 真实模型端到端冒烟测试与标准样例回归（158 个测试全绿，含真实游戏截图回归）
