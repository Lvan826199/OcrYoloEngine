...
- 可测试接缝：识别器抽象、依赖注入、真实数据测试与 golden 测试(详见 §7,不使用 mock/fake)。
...
- **依赖注入** `deps.py`：注册表、识别器、执行器通过 DI 提供，**测试可注入真实组件**(如真实 `TemplateRecognizer`)做端到端验证。
...
> 更新(2026-06-04):落地时采纳「**测试用真实数据(强制)**」规则,**不再使用 mock/fake**。原设计的 `tests/fakes/` 假实现方案已废弃,改为下述真实数据方案。

- **单元测试** `tests/unit/`：预处理(通道/ROI 回映射/上限)、错误契约、注册表 LRU、模板 NMS、坐标映射等,用**真实** numpy/cv2 图、真实线程、真实文件,不加载重模型。
- **契约测试** `tests/contract/`：HTTP API 用**真实 `TemplateRecognizer` + 真实模板图**端到端,校验请求/响应 schema、错误码、状态区分(有目标/无目标/失败)、鉴权、ROI 回映射、debug 标注图、`/metrics`。
- **golden 测试** `tests/fixtures/`：**入库**的固定样例图 + 期望结果(坐标/置信度带容差)，防模型/预处理改动悄悄改变行为。
- **冒烟测试** `tests/smoke/`：加载**真实模型**(PaddleOCR / yolov8n)跑端到端,打 `@pytest.mark.smoke`,`make smoke` 触发;重依赖缺失时 `importorskip` 优雅跳过。
- 识别器抽象 + DI 接缝保留(便于注入真实组件);CI 默认只跑快测(真实图像端到端),smoke 单独触发。
...
同一张图 + 同一组参数的识别请求在短时间内可能被重复发起(轮询、重试、前端抖动)。重复推理浪费 CPU/GPU。希望提供"结果缓存"省去重复计算,但**绝不能给默认部署带来任何隐性开销或行为变化**——核心管线在不开缓存时必须与现状逐字节一致。

### 考虑过的方案

- **不做缓存**:简单,但重复请求成本无法回收。
- **始终开启的内置缓存**:省事,但改变默认行为、引入哈希与内存开销,违背"默认零影响"。
- **可插拔 + 默认关闭(采纳)**:抽象 `ResultCache` 协议,默认装配 `NullResultCache`(get 恒 None、set/clear 空操作);`OYE_RESULT_CACHE_SIZE>0` 时才换成线程安全有界 LRU(可选 TTL)。管线用"外挂守卫"包裹核心计算,`result_cache_size==0` 时连缓存键的哈希都不算。

### 决策结果

采用可插拔后端 + 默认关闭。要点:

- 配置:`OYE_RESULT_CACHE_SIZE`(0=关闭,默认)、`OYE_RESULT_CACHE_TTL_S`(0=不过期)。
- 后端 `cache.py`:`NullResultCache`(关闭态)/ `LruResultCache`(`threading.Lock` + `OrderedDict`,TTL 用 `time.monotonic()`)。
- 缓存键:`sha256(解码后图像字节)` + 规范化参数串(排序后的 methods/templates、model、conf_threshold、roi),与来源(base64/path)无关;`debug` 请求不缓存。
- 请求级三模式 `cache`:`auto`(读+写)/`refresh`(强制算+写)/`off`(完全绕过);响应回 `from_cache` 与 `X-Cache` 头(HIT/MISS/BYPASS)。
- 失效策略:模型 `unload`/`reload` 端点在改完 registry 后调用 `cache.clear()`,**整体清空**(资产变更后旧结果一律作废,实现简单且安全;细粒度按键失效留作后续优化)。
- 指标:`oye_cache_events_total{event="hit|miss"}`。

实现见 `src/ocr_yolo_engine/cache.py`、`pipeline_runner.py`(守卫)、`service/{deps,routes}.py`。

#### 正面影响

- 默认零影响:`NullResultCache` + size==0 短路,核心管线无任何额外开销与行为变化。
- 开启后重复请求直接命中,显著省推理成本;LRU + TTL 控制内存与新鲜度。
- 可观测(X-Cache / from_cache / 指标),可按请求绕过或刷新。

#### 负面影响 / 代价

- 失效采用"全量清空"较保守,模型热管理后首批请求会重新计算(可接受;换来实现简单、无脏读风险)。
- 缓存键基于全图像素哈希,超大图哈希有成本——但仅在开启缓存时发生,且远小于一次推理。

## ADR 0004 - /recognize 多方法合并(可插拔策略)
...
`/v1/recognize` 可一次跑多种方法(ocr/yolo/template),但此前只把各方法结果分别放进 `method_results`,调用方若想要一份"跨方法的统一检测列表"需自行合并。常见诉求有三类:级联取首个命中(省后续算力)、跨方法去重、单纯汇总排序。需要内置这些策略,但**绝不能改变默认行为**——不传策略时必须与现状逐字段一致。

### 考虑过的方案

- **不做合并**:调用方自己合并,简单但重复造轮子、跨方法去重逻辑易写错。
- **固定一种合并(如总是去重)**:省事但不灵活,且改变默认输出。
- **可插拔策略 + 默认 none(采纳)**:`merge` 字段四选一(`none`/`priority`/`dedup`/`concat`),默认 `none` 即现状(`merged=None`,各方法分开返回);仅 `/recognize` 生效,单方法端点恒 `none`。

### 决策结果

采用可插拔策略 + 默认 `none`。要点:

- 数据模型:`RecognizeRequest.merge`(默认 `none`);`RecognizeResponse.merged: list[Detection] | None`(`none` 时为 `None`)。
- 纯函数策略放 `merge.py`:`concat`(汇总后按置信度降序)、`dedup`(汇总后按置信度降序做跨方法 NMS,IoU ≥ 0.5 视为同一目标)、`none`(返回 `None`);另有公开 `iou(a, b)`。
- `priority` **不在 `merge.py`**:它需要短路控制"跑几个方法",在 `pipeline_runner.py` 实现——按 `methods` 顺序逐个跑,遇首个非空 detections 即停,`method_results` 仅含已跑方法,`merged` = 命中方法的检测(全程无命中则 `[]`)。
- `none`/`concat`/`dedup` 维持"跑所有 methods"循环不变,算完后 `merged = merge_detections(merge, method_results)`。
- 缓存兼容:`CachedResult` 增 `merged` 字段;`compute_cache_key` 规范化参数串**纳入 `merge`**(merge 改变输出,不同 merge 不复用同一条目,避免串味);命中时把 `merged` 一并带回。
- debug 标注图逻辑不变(仍画 `method_results` 的框)。

实现见 `src/ocr_yolo_engine/merge.py`、`pipeline_runner.py`、`schemas.py`、`cache.py`。

#### 正面影响

- 默认 `none` 零影响:`merged=None`、`method_results` 与改动前逐字段一致。
- 内置三种常用合并,调用方免自行实现;`priority` 还能省下后续方法的算力。
- 与结果缓存正交且兼容:merge 入键,不同策略互不串味。

#### 负面影响 / 代价

- `priority` 与 `none`/`concat`/`dedup` 走两条控制流(短路 vs 全跑),`pipeline_runner` 略增分支复杂度(已抽出 `_run_single_method` 复用)。
- `dedup` 的 IoU 阈值固定 0.5,暂不可配(按需后续再开放)。
