...
缺失或错误返回 `401`。`/health`、`/ready`、`/metrics` 不需要鉴权。
...
| GET | `/metrics` | Prometheus 文本指标(各方法请求数/累计耗时) | — |
...
```jsonc
{
  ...
  "debug": false,                 // 可选,true 时返回标注图 debug_image
  "merge": "none"                 // 可选,仅 /recognize 生效:none|priority|dedup|concat
}
```

- `methods` 含 `yolo` 却没给 `model`,或含 `template` 却没给 `templates` → `422`(请求格式错误)。
- `roi`:只在图片的该矩形区域内识别;返回坐标会**自动加回偏移**,仍是相对**全图**的坐标。
- `merge`:多方法合并策略,**仅 `/v1/recognize` 生效**(单方法端点 `ocr`/`detect`/`match` 始终为 `none`);默认 `none`,详见「7. /recognize 多方法合并」。
...
```jsonc
{
  ...
  "debug_image": null,
  "merged": null                  // merge!=none 时为合并后的统一检测列表;none 时为 null
}
```

- `merged`:`merge` 为 `none`(默认)时恒为 `null`;为 `priority`/`dedup`/`concat` 时是一个**跨方法统一的 `Detection` 列表**(与 `method_results` 内的 detection 结构相同)。`method_results` 始终保留各方法的原始结果。
...
`/v1/recognize` 可一次跑多种方法,默认各方法结果分开放在 `method_results` 里。通过 `merge` 字段可额外得到一份**跨方法的统一检测列表** `merged`(单方法端点 `ocr`/`detect`/`match` 不受影响,始终 `none`)。

| 策略 | 行为 | `merged` |
|------|------|----------|
| `none`(默认) | 维持现状,各方法分开返回 | `null` |
| `priority` | 按 `methods` 顺序逐个跑,**遇到第一个有检出的方法就停**(后续方法不跑) | 该命中方法的检测;全程无命中则 `[]` |
| `dedup` | 汇总所有方法的检测,按置信度降序做跨方法 NMS(IoU ≥ 0.5 视为同一目标,只保留高分) | 去重后的列表 |
| `concat` | 汇总所有方法的全部检测,按置信度降序拼接 | 合并后的列表 |

- `priority` 适合"有 A 用 A,否则退而求其次用 B"的级联场景,且能省下后续方法的算力。
- `dedup` 适合多方法可能检到同一目标、需要一份去重结果的场景。
- `concat` 适合只想把所有检测汇成一条按置信度排序的列表。
- 无论哪种策略,`method_results` 都保留各方法的原始结果(`priority` 仅含已跑的方法)。
...
# method_results 里仍有 template、ocr 两块;merged 是二者全部检测按置信度降序的统一列表
```
...
# 若 yolo 有检出则不跑 template,merged = yolo 的检测;否则跑 template
```

> 提示:`merge` 会改变输出,因此也参与结果缓存键——同图不同 `merge` 互不串味。
...
| `OYE_RESULT_CACHE_SIZE` | `0` | 结果缓存容量(条数);**`0` 关闭**(默认),核心管线零开销 |
| `OYE_RESULT_CACHE_TTL_S` | `0` | 结果缓存条目存活秒数;`0` 表示不过期 |
...
### 9.2 模板登记 `configs/templates.yaml`
...
完全相同的输入(同图 + 同参数)在短时间内重复请求时,可命中缓存直接返回上次结果,省去重复推理。

- **默认关闭**:`OYE_RESULT_CACHE_SIZE=0` 时行为与现状完全一致,连缓存键的哈希都不计算,核心管线零开销。
- **开启**:设 `OYE_RESULT_CACHE_SIZE=N`(N>0)为容量上限,超出按最久未用(LRU)淘汰;可选 `OYE_RESULT_CACHE_TTL_S=T` 让条目 T 秒后过期。
- **缓存键**:基于解码后的图像像素 + 规范化请求参数(methods/model/templates/conf_threshold/roi)计算,与图片来源(base64/path)无关;`debug=true` 的请求不参与缓存。

每个请求可用 `cache` 字段(默认 `auto`)控制行为:

| 模式 | 行为 |
|---|---|
| `auto` | 读 + 写:先查缓存,命中即返回;未命中则计算并写入(默认) |
| `refresh` | 不读、强制重算,并把新结果写入(用于刷新过期数据) |
| `off` | 完全绕过缓存,既不读也不写 |

响应带 `X-Cache` 响应头与 `from_cache` 字段:

| `X-Cache` | 含义 |
|---|---|
| `HIT` | 命中缓存(`from_cache=true`) |
| `MISS` | 缓存开启但本次实际计算了(已写入) |
| `BYPASS` | 缓存关闭,或本次 `cache=off` |

**失效**:`POST /v1/models/{name}/unload`、`reload` 会清空整个结果缓存(资产变更后旧结果作废)。
