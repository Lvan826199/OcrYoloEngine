...
### train.py 默认 device="0" 在无 GPU 机器必崩

- **现象**:`training/train.py` 不带 `--device` 时默认 `"0"`(GPU 0),纯 CPU 机器上 ultralytics 直接报错退出,新手按旧 README 命令第一步就失败。
- **根因**:骨架期写死 GPU 默认值,从未在无 GPU 环境真实跑过。
- **修法**:默认改 `None`(ultralytics 自动选择:有 GPU 用 GPU,否则 CPU);训练完打印最优权重路径与接入服务三步提示。
- **验证**:CPU 环境真实训练跑通(demo 数据集 40 轮 2.2 分钟,mAP50 两类均 0.995),自训模型经真实服务 `/v1/detect` 检出目标,坐标与真值偏差 <0.005。
